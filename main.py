from local_colorama import *; import os
used_texts = []
...
searchterm=input(Fore.RESET + 'Search term: ' + Fore.BLUE).split(', ')# ; print(f'{things}\n{thingys}\n')
...
for i in range(len(searchterm)):
    if case_sesitive and searchterm[i].split(f' ({searchterm[i].split(" (")[1].rstrip(")")})')[0] in database and searchterm[i].split(f' ({searchterm[i].split(" (")[1].rstrip(")")})')[0] not in used_texts: print(Fore.GREEN + 'Yes' + Fore.WHITE + ': \'' + Fore.BLUE + f'{searchterm[i]}' + Style.RESET+ '\' Is in the list') 
    elif not case_sesitive and searchterm[i] in database and searchterm[i] not in used_texts: print(Fore.GREEN + 'Yes' + Fore.WHITE + ': \'' + Fore.BLUE + f'{searchterm[i]}' + Style.RESET+ '\' Is in the list') 
    else:
